...
test('left category list supports card drops and category reordering',()=>{assert.match(html,/class="category-workspace"/);assert.match(html,/aria-label="項目一覧"/);assert.match(app,/application\/x-step-category/);assert.match(app,/reorderCategory\(/);assert.match(app,/moveCategory\(/);assert.match(app,/moveApp\(event\.dataTransfer\.getData\('text\/plain'\),category\.id\)/)});
