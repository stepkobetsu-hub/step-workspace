...
test('left list has search and accepts explicit app card drops',()=>{assert.match(html,/id="categorySearch"/);assert.match(html,/項目・カード名を検索/);assert.match(app,/application\/x-step-app/);assert.match(app,/filterCategoryNavigation\(/);assert.match(app,/categoryNavEmpty/)});
