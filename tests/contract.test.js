...
test('スタッフ共通認証と二つの台帳正本を再利用する',()=>{assert.match(app,/api\('staffLogin'/);assert.match(app,/api\('getSystemRegistry'\)/);assert.match(app,/SYSTEM_REGISTRY\.md/);assert.match(app,/mergeRegistrySources/);assert.match(app,/stepStaffAppAuth/);assert.match(app,/\['2','3','4'\]/)});
