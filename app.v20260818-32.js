(function(){
  ...
  function rebuildApps(){
    ...
    state.apps=Core.applyWorkspaceConfig(state.allApps.filter(app=>!state.config.archived.includes(app.id)),state.config).map(applyRegistryCardDetails);
  }
  ...
  function registryAppsFromShared(payload){
    ...
    const source=registry.customCards.filter(item=>item?.id&&!archived.has(`custom:${item.id}`)&&item.url).map(item=>({id:`registry-user-${item.id}`,displayName:item.title||'追加カード',description:item.summary||'システム資産台帳から同期',category:registryPurposeCategory(item.purpose),productionUrl:item.url,parentSystem:'STEPシステム資産台帳',audiences:Array.isArray(item.audiences)?item.audiences:[],keywords:[...(Array.isArray(item.audiences)?item.audiences:[]),item.purpose].filter(Boolean),isNew:true,favorite:true,recent:true,status:'active'}));
    return Core.buildApps(source,{allowDuplicateUrls:true}).map(app=>Object.assign({},app,{audiences:app.source.audiences||[]}));
  }
  ...
  function registryCardOverridesFromShared(payload){const result={ids:{},names:{}};const cards=payload?.registryConfig?.cards;if(!cards||typeof cards!=='object')return result;for(const [key,override] of Object.entries(cards)){if(!override||typeof override!=='object')continue;const value={};if(Object.prototype.hasOwnProperty.call(override,'title'))value.name=String(override.title||'');if(Object.prototype.hasOwnProperty.call(override,'summary'))value.description=String(override.summary||'');if(Object.prototype.hasOwnProperty.call(override,'url'))value.url=String(override.url||'');if(Array.isArray(override.audiences))value.audiences=override.audiences.map(String).filter(Boolean);if(override.purpose)value.purpose=String(override.purpose);if(key.startsWith('id:')){const id=key.slice(3),app=state.baseApps.find(item=>item.id===id);result.ids[id]=value;if(app)result.names[Core.normalize(app.name)]=value}else if(key.startsWith('name:'))result.names[Core.normalize(key.slice(5).split('|')[0])]=value}return result}
  function automaticAudiences(app){const source=`${app.name||''} ${app.description||''} ${app.parentSystem||''}`;const values=[];if(/講師|先生/.test(source))values.push('講師');if(/生徒|塾生|子供/.test(source))values.push('生徒');if(/スタッフ|受付/.test(source))values.push('スタッフ');if(/管理者|所有者|保護者|経理/.test(source))values.push('管理者');if(!values.length)values.push(/ホームページ|お問い合せ|問い合わせ|紹介/.test(source)?'その他':'スタッフ');return [...new Set(values)]}
  function registryOverrideForApp(app){const original=Core.normalize(Core.nameOf(app.source||{}));return state.registryCardOverrides.ids[app.id]||state.registryCardOverrides.names[original]||state.registryCardOverrides.names[Core.normalize(app.name)]||null}
  function applyRegistryCardDetails(app){const override=registryOverrideForApp(app);const name=override&&Object.prototype.hasOwnProperty.call(override,'name')?override.name:app.name;const description=override&&Object.prototype.hasOwnProperty.call(override,'description')?override.description:app.description;const url=override&&Object.prototype.hasOwnProperty.call(override,'url')?override.url:app.url;const audiences=override?.audiences?.length?override.audiences:(app.audiences?.length?app.audiences:automaticAudiences(app));return Object.assign({},app,{name,description,url,productionUrl:url,audiences,searchText:Core.normalize([name,description,app.parentSystem,app.categoryLabel,...audiences].join(' '))})}
  ...
  function createCard(app,context){
    ...
    renderAppIcon(card.querySelector('.app-icons'),app);const newBadge=card.querySelector('.new-badge');newBadge.hidden=!app.isNew;card.querySelector('.app-copy strong').textContent=app.name;const description=card.querySelector('.app-copy small');description.textContent=app.description;description.title=app.description;const cardUrl=card.querySelector('.card-url');if(app.id===REQUIRED_BILLING_ADJUSTMENT_APP.id){cardUrl.hidden=false;cardUrl.textContent=app.url;cardUrl.title=app.url}const audienceRoot=card.querySelector('.app-audiences');audienceRoot.hidden=!app.audiences?.length;audienceRoot.replaceChildren(...(app.audiences||[]).map(name=>{const badge=document.createElement('i');const type=state.sharedEnvelope?.registryConfig?.audienceTypes?.find(item=>item.name===name);badge.textContent=`${type?.icon||'●'} ${name}`;if(type?.color)badge.style.setProperty('--audience-color',type.color);return badge}));card.querySelector('.app-category-tag').textContent=app.categoryLabel;
  ...
  byId('usageButton').addEventListener('click',()=>byId('usageDialog').showModal());
