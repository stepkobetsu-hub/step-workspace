(function(root,factory){
  ...
  function urlKey(value){try{const u=new URL(text(value));u.searchParams.delete('v');return u.href.replace(/\/$/,'')}catch(_){return text(value)}}
  ...
  function normalizeWorkspaceConfig(value){
    ...
    const cardOverrides={};for(const [id,item] of Object.entries(source.cardOverrides&&typeof source.cardOverrides==='object'?source.cardOverrides:{})){if(!id||!item||typeof item!=='object')continue;const name=text(item.name);const description=text(item.description);const url=text(item.url);const googleSheet=typeof item.googleSheet==='boolean'?item.googleSheet:undefined;const clean={};if(name)clean.name=name;if(description)clean.description=description;if(isUrl(url))clean.url=url;if(googleSheet!==undefined)clean.googleSheet=googleSheet;if(Object.keys(clean).length)cardOverrides[id]=clean}
  ...
  function applyWorkspaceConfig(apps,value){
    const config=normalizeWorkspaceConfig(value);return (Array.isArray(apps)?apps:[]).map(app=>{const category=categoryDefinition(config.assignments[app.id]||app.categoryId,config.categories);const override=config.cardOverrides[app.id]||{};const name=override.name||app.name;const description=override.description||app.description;const requestedUrl=override.url||app.url;const billingManagement=app.id==='billing'||app.id==='billing-special-adjustment'||/請求管理システムV?3\.1|料金特別調整/.test(name);const invoiceDelivery=app.id==='invoice-pdf'||/STEP請求書PDF|請求書作成・送信・入金管理|請求書作成・配信/.test(name);const url=billingManagement?'https://stepkobetsu-hub.github.io/invoice-pdf/#adjustments':invoiceDelivery?'https://stepkobetsu-hub.github.io/invoice-pdf/#invoices':requestedUrl;const iconType=override.googleSheet===undefined?(isGoogleSheetUrl(url)?'google-sheet':app.iconType):(override.googleSheet?'google-sheet':'category');return Object.assign({},app,{name,description,url,productionUrl:url,iconType,searchText:normalize([name,description,app.parentSystem,category.label].join(' ')),categoryId:category.id,categoryLabel:category.label,categoryClass:category.className,initial:category.initial,categoryIcon:category.icon,categoryColor:category.color,categorySoftColor:category.softColor,device:config.devices[app.id]||'both'})});
  }
