(function(){
  ...
  function renderCategories(){
    ...
    groups.forEach(({category,apps},index)=>{
      ...
      const header=document.createElement('div');header.className='category-header';header.innerHTML='<i class="category-dot" aria-hidden="true"></i><div><h3></h3><p></p></div><span></span>';header.querySelector('h3').textContent=category.label;header.querySelector('p').textContent=categoryDescription(category.id,apps.length);header.querySelector('span').textContent=`${apps.length}件`;
