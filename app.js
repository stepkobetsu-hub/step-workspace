(function(){
  ...
  function appIconSvg(app){const value=`${app.name} ${app.description}`;let path='<path d="M4 5h16v14H4z"/><path d="M8 9h8M8 13h5"/>';if(/QR|入退|出退/.test(value))path='<path d="M4 4h6v6H4zM14 4h6v6h-6zM4 14h6v6H4z"/><path d="M14 14h2v2h-2zM18 14h2v6h-6v-2"/>';else if(/請求|会計|給与|学費|領収/.test(value))path='<path d="M6 3h12v18l-3-2-3 2-3-2-3 2Z"/><path d="M9 8h6M9 12h6M9 16h4"/>';else if(/配信|メール|連絡|問い合わせ/.test(value))path='<path d="M3 5h18v14H3z"/><path d="m3 7 9 7 9-7"/>';else if(/成績|進捗|分析|データ/.test(value))path='<path d="M4 19V9M10 19V5M16 19v-7M22 19H2"/>';else if(/講師|生徒|面談|授業/.test(value))path='<circle cx="9" cy="8" r="3"/><path d="M3 20c0-4 2-7 6-7s6 3 6 7M17 8h4M19 6v4"/>';else if(/ポイント/.test(value))path='<path d="m12 3 2.7 5.5 6.1.9-4.4 4.3 1 6.1-5.4-2.9-5.4 2.9 1-6.1-4.4-4.3 6.1-.9Z"/>';return `<svg viewBox="0 0 24 24" aria-hidden="true">${path}</svg>`}
  function categoryIconSvg(id){const paths={student:'<circle cx="9" cy="8" r="3"/><path d="M3 20c0-4 2-7 6-7s6 3 6 7M17 8h4M19 6v4"/>',contact:'<path d="M4 5h16v11H9l-5 4Z"/><path d="M8 9h8M8 12h5"/>',billing:'<path d="M4 7h16v12H4z"/><path d="M4 10h16M15 15h2"/>',teacher:'<circle cx="9" cy="8" r="3"/><path d="M3 20c0-4 2-7 6-7s6 3 6 7M17 5h4v8h-4z"/>',admin:'<circle cx="12" cy="12" r="3"/><path d="M12 2v3M12 19v3M2 12h3M19 12h3M5 5l2 2M17 17l2 2M19 5l-2 2M7 17l-2 2"/>'};return `<svg viewBox="0 0 24 24" aria-hidden="true">${paths[id]||'<path d="M4 4h6v6H4zM14 4h6v6h-6zM4 14h6v6H4zM14 14h6v6h-6z"/>'}</svg>`}
  ...
  function createCard(app,context){
    ...
    if(state.organizing&&context==='category'){card.draggable=true;card.classList.add('is-organizing');card.addEventListener('dragstart',event=>{card.classList.add('is-dragging');event.dataTransfer.setData('application/x-step-app',app.id);event.dataTransfer.setData('text/plain',app.id);event.dataTransfer.effectAllowed='move'});card.addEventListener('dragend',()=>card.classList.remove('is-dragging'))}
    renderAppIcon(card.querySelector('.app-icons'),app);card.querySelector('.app-copy strong').textContent=app.name;card.querySelector('.app-copy small').textContent=app.description;card.querySelector('.app-category-tag').textContent=app.categoryLabel;
  ...
  function renderCategories(){
    ...
    groups.forEach(({category,apps},index)=>{
      const navItem=document.createElement('div');navItem.className=`category-nav-item ${category.className}`;navItem.dataset.categoryId=category.id;navItem.dataset.search=Core.normalize([category.label,...apps.map(app=>app.name)].join(' '));
      const anchor=document.createElement('a');anchor.href=`#category-${category.id}`;anchor.innerHTML='<i aria-hidden="true"></i><span></span><small></small>';anchor.querySelector('i').innerHTML=categoryIconSvg(category.id);anchor.querySelector('span').textContent=category.label;anchor.querySelector('small').textContent=apps.length;anchor.addEventListener('click',()=>{tabs.querySelectorAll('.category-nav-item').forEach(item=>item.classList.remove('is-active'));navItem.classList.add('is-active');closeMobileMenu()});if(index===0)navItem.classList.add('is-active');
      ...
      handle.addEventListener('dragstart',event=>{navItem.classList.add('is-dragging');event.dataTransfer.setData('application/x-step-category',category.id);event.dataTransfer.effectAllowed='move'});handle.addEventListener('dragend',()=>navItem.classList.remove('is-dragging'));
      ...
      const header=document.createElement('div');header.className='category-header';header.innerHTML='<i class="category-dot" aria-hidden="true"></i><div><h3></h3><p></p></div><span></span>';header.querySelector('.category-dot').innerHTML=categoryIconSvg(category.id);header.querySelector('h3').textContent=category.label;header.querySelector('p').textContent=categoryDescription(category.id,apps.length);header.querySelector('span').textContent=`${apps.length}件`;
