(function(){
  ...
  function rebuildApps(){
    const custom=Core.buildApps(state.config.customApps);const catalog=state.config.replaceCatalog?[]:state.baseApps;const seen=new Set();state.allApps=[...custom,...catalog].filter(app=>{if(seen.has(app.id)||state.config.deleted.includes(app.id))return false;seen.add(app.id);return true});
  ...
  async function refreshSharedConfig(){
    if(!readAuth()||state.sharedLoading||state.sharedPublishing)return;state.sharedLoading=true;
